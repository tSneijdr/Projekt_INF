...
public class Octree {
	...
	public Set<Point> getPoints(Range3D searchRange) {
	...
	private class OctreeNode {
		...
		private OctreeNode[] subtrees = { NWE, NEE, SWE, SEE, NWL, NEL, SWL, SEL };
		...
		public void add(Point p) {
			...
			if (NEE != null && NWE != null && SEE != null && SWE != null && NEL != null && NWL != null && SEL != null
					&& SWL != null) {
		...
		public Set<Point> getPoints(Range3D searchRange) {
			...
			if (hasSubtrees()) {
				...
				// existent
				for (OctreeNode node : this.subtrees) {
		...
		private void initializeSubtrees() {
			...
			// nicht geschehen ist
			if (points.size() > maxNumElements && !hasSubtrees()) {
